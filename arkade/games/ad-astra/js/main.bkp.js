...
class Game {
    ...
    async handleAutoLogin() {
        ...
        if (success) {
            ...
            // If pilotName is missing, empty, or 'Unknown', treat as new player
            // But if gameState exists and has substance, we trust it more
            const hasValidPilot = user.pilotName && user.pilotName !== 'Unknown';
            const hasGameState = user.gameState && Object.keys(user.gameState).length > 0;

            if (!hasValidPilot && !hasGameState) {
